%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% plot/fit to Omicron and Delta data from the ABOO site
...
% ABOO data
V_AB_O = ABflows.*ABrna_Omicron;
V_AB_D = ABflows.*ABrna_Delta;
...
allData = [ABdays V_AB_D V_AB_O];
allData = allData(split:end,:);

ABdays = allData(:,1);
V_AB_D_new = allData(:,2);
V_AB_O_new = allData(:,3);
...
problem = createOptimProblem('fmincon','x0',p0,...
    'objective',@(param)obj_fun(param,ABdays,V_AB_D_new,V_AB_O_new),'lb',lb,'ub',ub,'Aineq',A,'bineq',b);

% global search with multistart
...
%Note, we only model 2nd strain.
I10 = 0;
I20 = V_AB_O_new(1)/best_params(5);
R0 = 0;
S0 = N0 - (I10 + I20 + R0);               
ICs  = [S0 I10 I20 R0 V_AB_D_new(1) V_AB_O_new(1) I10+I20];

tspan = ABdays(1):1:ABdays(end);
...
tspan_fwd_bs = ABdays(1):1:ABdays(end);
[~,Y0_bs] = ode45(@SIIR,tspan_fwd_bs,ICs,[],best_params);

ind_bs = ABdays - ABdays(1);
ind_bs = ind_bs(2:end);
...
obs_log_bs = log10(V_AB_O_new(2:end));
...
for bb = 1:B
    ...
    data2_star = V_AB_O_new;
    data2_star(2:end) = max(eps, 10.^ystar_log);
    
    % Refit on bootstrap sample
    problem_b = createOptimProblem('fmincon','x0',p0,...
        'objective',@(param)obj_fun(param,ABdays,V_AB_D_new,data2_star),'lb',lb,'ub',ub,'Aineq',A,'bineq',b);
    ...
    ICs_b = [S0_b I10_b I20_b R0_b V_AB_D_new(1) data2_star(1) I10_b+I20_b];
    
    tspan_f = ABdays(1):1:ABdays(end);
    [~,Yb_f] = ode45(@SIIR,tspan_f,ICs_b,[],param_b);
    tspan_r = ABdays(1):-1:1;
...
figure(); box on; hold on;
    ...
    plot(ABflowdates(split-1:end),V_AB_O(split-1:end),'.','LineWidth',3,'MarkerSize',20,'color',br);
    ...
    namess = strcat('ABOO_',num2str(frac_S),'_',num2str(data_used));
    exportgraphics(f1,strcat(namess,'.png'),'Resolution',300)
    write(T_boot,'Bootstrap_ABOO.xlsx','Sheet',1)
...
%Note, we only model 2nd strain.
    ...
    gamma1 = 1/8;
    gamma2 = 1/8;
    a = 0;%neglect demography for short term
    d = 0;%neglect demography for short term

    % per capita vaccinations (see vax_data.m)
    v = 0;%neglect vaccination effect for short term
